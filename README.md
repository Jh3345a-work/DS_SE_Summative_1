...
## Plan
The Agile methodology was selected for this project as the full set of requirements was not fully defined at the outset. Instead, the initial goal was to retrieve population data from the NOMIS API, create several data visualisations, and deploy them within an app. Using the Agile method benefitted me during this project as I was able to pivot from creating a Tkinter app to deploying an online Streamlit app, when it became more suitable.

Following this, I created a GitHub project to manage and record all my issues. Specifically, I chose a Kanban board to track the progression of every issue and pull request. I predominantly utilised the Backlog, In Progress, and Done columns. The In Review column was retained in the project but was not used as I was the only developer in this project. In future, if I was to work collaboratively with a colleague in similar software development, this section would house latest developments prior to final sign-off.

An example of the Kanban board used can be found in the screenshot below, clearly demonstrating the untouched tasks and issues in the backlog, the current issues in progress, and the stack of issues which are now closed and completed. Additionally, GitHub projects allowed me to mark the priority, estimated size, and sprint of each individual issue. The following screenshot then shows the progression of this documentation ticket once it had been completed.

![kanban board](images/kanban.png)

![kanban board progression](images/kanbanprogression.png)

My project was split into several individual sprints. Sprint 1 covered issues such as “Create Project Proposal” and “Design Figma Prototype” which were tasks which could be picked up early in the project. Future sprints often enhanced work committed in earlier sprints, such as the “Create Data Visualisations” from Sprint 3 which built on the work committed by “Create Filtered Dataframe Function” from Sprint 2. By iterating in small sections, rather than significant sections of code, testing and successful implementation became easier. 
...
Instruction on how to access the app can be found within the User Documentation section.
...
## User Documentation
The NOMIS Population Estimates Visualision app can be found [here](https://dssesummative1-hgkh2w98wq8w2go2caj46u.streamlit.app/). 

The app is simple to use. Select any of the three tabs to view different data visualisations. The app automatically retireves population estimate data for 2023 and 2024 directly from the NOMIS API.
...
## Evaluation
To conclude, this section evaluates both the final product and the project as a whole.

The final product, a deployed Streamlit Python app, has been a major success. It presents three simple data visualisations using data from the online NOMIS API. The product has had many iterations, evolving from simple code in a Jupyter notebook, to a Tkinter prototype, to the final publicly deployed Streamlit app. The final product meets all project requirements and provides a useful tool to visualise population estimate across regions.

Reflecting on the project, I am happy with the knowledge and skills gained. My understanding of user requirements has improved significantly, supported by tools such as the Empathy map and Prototype Feedback form. This process has helped me appreciate the importance of managing user expectations and incorporating user-centred design throughout development. 
My GitHub skills and familiarity with Agile project management tools have also improved. Although I initially found commits and version control challenging, my competence increased over time, as reflected in clearer commit messages, better labelling, and more consistent documentation. Additionally, I am now much more confident using prototyping tools like Figma, which I will continue to use for future software development. 
