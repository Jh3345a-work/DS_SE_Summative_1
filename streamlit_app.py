...
def filter_population_df(df: pd.DataFrame) -> pd.DataFrame:
    '''
    This function filters and cleans the population DataFrame to include only region-level data
...
def fig_population_by_region(df_year: pd.DataFrame, year: int):
    '''
    This function creates a bar chart showing the population by region for a given year.
    '''
...
def fig_population_share_change(df23: pd.DataFrame, df24: pd.DataFrame):
    '''
    This function creates a horizontal bar chart showing the change in population share by region.
    '''
...
if __name__ == "__main__":
    ...
    '''
    This designs the UI and runs the Streamlit app to visualise NOMIS population estimates data.
    '''
